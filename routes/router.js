...
router.post("/setProfilePic", upload.single("image"),function (req, res, next) {
	  let image_uuid = uuid();
	  let user_id = req.body.user_id;
	  let buf64 = req.file.buffer.toString("base64");
	  stream = cloudinary.uploader.upload("data:image/octet-stream;base64," + buf64,
...
router.get("/deleteProfileImage", async (req, res) => {
	try {
	  ...
	  if (validationResult.error != null) {
		...
		res.render("error", { message: "Invalid user_id -joi error-" });
		return;
	  }
	  ...
	  if (user_id) {
		...
		if (!success) {
		  res.render("error", {
			message: "Error connecting to MongoDB I assume?",
		  });
		  return;
		}
	  }
...
router.post('/addUser', upload.single('profile_image'), async (req, res) => {
    try {
